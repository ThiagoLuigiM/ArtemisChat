...
| `extract_form_fields` | `ticket_text: String` | `FormFieldsSuggestion` (10 campos camelCase) | IA extrai os campos do FormView do texto bruto de um ticket. Campos sem evidência voltam vazios; frontend mescla preservando o que o usuário digitou |
| `stream_revision` | `user_text: String` | — | Modo revisão: aplica estilo/evitar/campos-padrao a uma devolutiva escrita à mão, sem reescrever. Reusa eventos `deepseek-token`/`done`; aprovar/descartar usam o texto colado como raw_input |
| `learning_stats` | — | `LearningStats { total, approved, discarded, edited_approved, weeks, categories }` | Dashboard: taxa de edição por semana ISO + volume por categoria. Parsing puro do SQLite, sem IA |
| `export_cartilha_pdf` | `index_html_path: String` | `String` (path do PDF) | Gera `cartilha.pdf` ao lado do index.html via Edge headless `--print-to-pdf` (CSS @media print aplicado). Sem deps novas |
| `get_hotkey` / `set_hotkey` | `hotkey: String` (formato canônico "Ctrl+Shift+KeyD") | `String` / — | Hotkey global configurável: valida, desregistra tudo, registra a nova (rollback pra anterior se falhar) e persiste em config.json |
| `get_vault_git_backup` / `set_vault_git_backup` | `enabled: bool` | `bool` / — | Toggle do backup git automático do vault. Ao ligar, roda backup inicial (erro se git ausente). `maybe_git_backup()` é chamado após toda escrita do app no vault |
...
### ✅ Rodada de 10 funcionalidades (concluída 2026-07-15)
Todas aprovadas pelo usuário ("vamos aplicar todas uma por vez"):
1. **Preencher formulário do ticket** — `deepseek::extract_form_fields` (JSON camelCase, temp 0.2, proibida de inventar) + área "⚡ Preencher a partir do ticket" no topo do FormView; `mergeExtractedFields` preserva o que o usuário digitou e valida os selects.
2. **Painel de histórico (🕘 no header)** — `HistoryPanel` com busca FTS5 (debounce 250ms), filtro "só aprovadas", chips de categoria/status/editada, expandir com final_output + raw_input, copiar e excluir (confirmação em 2 cliques). Backend já existia.
3. **Rascunho persistente do FormView** — localStorage (`artemis-form-draft`), restaura no mount (lazy initial state), persiste com debounce 400ms; aprovar/descartar/limpar zera.
4. **Índice de cartilhas** — `vault::regenerate_cartilhas_index` regenera `cartilhas/index.html` (cards com título do `<title>` + data do prefixo da pasta, mais recente primeiro) após cada `save_cartilha`. Best-effort.
5. **Exportar PDF** — botão na tela de sucesso da cartilha; Edge headless (vide tabela de comandos).
6. **Passos numerados e tabelas na cartilha** — bloco de linhas `1.`/`1)` (≥2) vira `.steps` com círculos azuis (prefixo removido); bloco de linhas com `|` (≥2) vira `.table-wrap` (primeira linha = cabeçalho; separador `|---|` descartado). Prompt atualizado com as convenções.
7. **Dashboard de estatísticas** — 5ª tab "estatísticas" no LearningPanel: cards (aprovadas/descartadas/taxa de edição), barras semanais (parte âmbar = editadas; meta é ela sumir) e por categoria. `History::list_stats_meta` (metadados leves, sem textos).
8. **Modo revisão** — botão "Revisar" no footer do FormView → `RevisaoView` (colar texto) → `prompt::build_revision_messages` (regras do vault SEM o template de geração; preservar estrutura) → ResultView normal.
9. **Hotkey configurável** — campo de captura no Settings (usa `e.code` W3C = nomes do `keyboard_types::Code`; exige modificador). Formato canônico `Ctrl+Shift+KeyD` em config.json; handler do plugin dispara para qualquer atalho registrado (só registramos um).
10. **Backup git do vault** — `vault::git_backup` (init na 1ª vez; add -A + commit com identidade local `-c user.name=Artemis` e `commit.gpgsign=false` pra não travar em pinentry; "nothing to commit" não é erro; `CREATE_NO_WINDOW` pra não piscar console). Toggle no Settings; `maybe_git_backup` após: approve_entry, apply_evitar/estilo/campos/frases, save_cartilha, seed_vault.

Config.json ganhou os campos `hotkey` e `vault_git_backup`. ⚠️ Rust não compilado localmente nesta rodada (sem toolchain na máquina C:\Space) — `cargo check`/`cargo test` pendentes no CI; `tsc --noEmit` 0 erros após cada item.

### ⏳ Próximas fases pendentes
(vazio — roadmap anterior concluído)
